...
# robot_radius = 0.05 #0.045 #0.05
# final_tolerance = 0.01
# D = robot_radius*0.35 #0.35
...
time_step = 2.0 #2.0 # 0.02
velocity_time_step = 1.2 # 0.95
message_time = 0.05 # 0.05
robot_radius = 0.048 #0.045 #0.05
final_tolerance = 0.01
D = robot_radius*0.33 #0.35
L = (robot_radius*2) - 0.04
effective_robot_radius = robot_radius+D
message_radius = robot_radius*4 #4 #8
responsibility = 0.3
grid_resolution = 0.1
grid_bound = 4 # 6
expanded_grid_resolution = 0.2
expanded_grid_bound = 25
...
def usr(robot):
    ...
    while True:
        ...
        desired_pose = temp_poses(robot.virtual_id()) # N_init_positions_skinny(robot, robot.virtual_id())
        ...
        # send robot location
        # robot.send_msg(struct.pack('ifffff', robot.virtual_id(), curr_x, curr_y, curr_theta, own_opt_x, own_opt_y))
        ...
        while(robot.get_clock()-last) < message_time:
            for i in range(2):             
                ...
                # to_send = package_string([robot.virtual_id(), curr_x, curr_y, curr_theta, own_vopt_x, own_vopt_y])
                # did_send = robot.send_msg(to_send)
                # # robot.logger.info(did_send)
                # robot.delay()
                if first_flag == 1:
                    robot.send_msg(struct.pack('ifffff', robot.virtual_id(), curr_x, curr_y, curr_theta, x_dot, y_dot))
                else:
                    robot.send_msg(struct.pack('ifffff', robot.virtual_id(), curr_x, curr_y, curr_theta, (effective_x_dot), (effective_y_dot)))
                robot.delay()
            first_flag = 0
        ...
        if incoming_message_count == 0:
            no_other_robots_active_flag = no_other_robots_active_flag + 1
        else:
            no_other_robots_active_flag = 0
        ...
        if invalid == 1:
            ...
            effective_x_dot = 0
            effective_y_dot = 0
            robot.send_msg(struct.pack('ifffff', robot.virtual_id(), curr_x, curr_y, curr_theta, 0, 0))
            ...
            if leader != robot.virtual_id():
                turn_to_angle(robot, curr_theta, final_theta)
                robot.send_msg(struct.pack('ifffff', robot.virtual_id(), curr_x, curr_y, curr_theta, 0, 0))
                return
            elif (leader == robot.virtual_id()) and (no_other_robots_active_flag > 10):
                robot.logger.info("is leader robot and received no messages")
                turn_to_angle(robot, curr_theta, final_theta)
                robot.send_msg(struct.pack('ifffff', robot.virtual_id(), curr_x, curr_y, curr_theta, 0, 0))
                return
        else:
...
def calculate_wheel_speeds(robot, position_filter_prev, desired_vel, curr_pose, prev_pose, vive_update_rate, integrated_error, speed_filter):
    ...
    robot.logger.info(str(robot.virtual_id()) + " : new wheel speeds " + str(vl) + " , " + str(vr))
...
def get_init_pose(id):
	init_pose = {}
	init_pose[0] = [-0.255, -2.1, 1.5707963267948966]
	init_pose[1] = [-0.11000000000000004, -2.1, 1.5707963267948966]
	init_pose[2] = [-1.36, -1.197, 0]
	init_pose[3] = [0.9049999999999999, -2.1, 1.5707963267948966]
	init_pose[4] = [-1.36, -0.5169999999999999, 0]
	init_pose[5] = [-1.4, 0.428, 0]
	init_pose[6] = [1.0499999999999998, -2.1, 1.5707963267948966]
	init_pose[7] = [-0.95, 0.6500000000000001, 0]
	init_pose[8] = [-0.95, 0.97, 0]
	init_pose[9] = [-1.4, 1.128, 0]
	init_pose[10] = [-1.4, 0.778, 0]
	init_pose[11] = [1.1949999999999998, -2.1, 1.5707963267948966]
	init_pose[12] = [-0.6399999999999999, -1.59, 0]
	init_pose[13] = [-0.6399999999999999, -1.27, 0]
	init_pose[14] = [-1.36, -1.027, 0]
	init_pose[15] = [-1.36, -0.6869999999999999, 0]
	init_pose[16] = [-1.36, -0.347, 0]
	init_pose[17] = [-1.4, 0.603, 0]
	init_pose[18] = [-0.6399999999999999, 0.32999999999999985, 0]
	init_pose[19] = [-0.6399999999999999, 0.6500000000000001, 0]
	init_pose[20] = [-1.4, 0.9529999999999998, 0]
	init_pose[21] = [-0.6399999999999999, 1.2899999999999998, 0]
	init_pose[22] = [-1.4, 1.6529999999999998, 0]
	init_pose[23] = [-0.6399999999999999, 1.93, 0]
	init_pose[24] = [-0.4, -2.1, 1.5707963267948966]
	init_pose[25] = [-0.32999999999999996, -1.27, 0]
	init_pose[26] = [-1.36, -0.857, 0]
	init_pose[27] = [0.17999999999999994, -2.1, 1.5707963267948966]
	init_pose[28] = [-0.32999999999999996, -0.31000000000000005, 0]
	init_pose[29] = [-0.32999999999999996, 0.010000000000000009, 0]
	init_pose[30] = [-0.32999999999999996, 0.32999999999999985, 0]
	init_pose[31] = [0.32499999999999996, -2.1, 1.5707963267948966]
	init_pose[32] = [-0.32999999999999996, 0.97, 0]
	init_pose[33] = [-0.32999999999999996, 1.2899999999999998, 0]
	init_pose[34] = [-1.4, 1.8279999999999998, 0]
	init_pose[35] = [-1.4, 2.003, 0]
	init_pose[36] = [0.03499999999999992, -2.1, 1.5707963267948966]
	init_pose[37] = [0.46999999999999986, -2.1, 1.5707963267948966]
	init_pose[38] = [0.6149999999999999, -2.1, 1.5707963267948966]
	init_pose[39] = [-0.020000000000000018, -0.6300000000000001, 0]
	init_pose[40] = [-1.36, -0.17699999999999982, 0]
	init_pose[41] = [-1.36, -0.006999999999999895, 0]
	init_pose[42] = [-1.36, 0.16300000000000003, 0]
	init_pose[43] = [-1.36, -1.367, 0]
	init_pose[44] = [-0.020000000000000018, 0.97, 0]
	init_pose[45] = [-1.4, 1.303, 0]
	init_pose[46] = [-1.4, 1.4779999999999998, 0]
	init_pose[47] = [-0.020000000000000018, 1.93, 0]
	init_pose[48] = [0.29000000000000004, -1.59, 0]
	init_pose[49] = [0.7599999999999999, -2.1, 1.5707963267948966]
	return init_pose[id]


def add_parking_spots():
	parking_spots = {}
	parking_spots[0] = [-0.255, -2.1, 1.5707963267948966, 0, 0]
	parking_spots[1] = [-0.11000000000000004, -2.1, 1.5707963267948966, 0, 0]
	parking_spots[2] = [-1.36, -1.197, 0, 0, 0]
	parking_spots[3] = [0.9049999999999999, -2.1, 1.5707963267948966, 0, 0]
	parking_spots[4] = [-1.36, -0.5169999999999999, 0, 0, 0]
	parking_spots[5] = [-1.4, 0.428, 0, 0, 0]
	parking_spots[6] = [1.0499999999999998, -2.1, 1.5707963267948966, 0, 0]
	parking_spots[7] = [-0.95, 0.6500000000000001, 0, 0, 0]
	parking_spots[8] = [-0.95, 0.97, 0, 0, 0]
	parking_spots[9] = [-1.4, 1.128, 0, 0, 0]
	parking_spots[10] = [-1.4, 0.778, 0, 0, 0]
	parking_spots[11] = [1.1949999999999998, -2.1, 1.5707963267948966, 0, 0]
	parking_spots[12] = [-0.6399999999999999, -1.59, 0, 0, 0]
	parking_spots[13] = [-0.6399999999999999, -1.27, 0, 0, 0]
	parking_spots[14] = [-1.36, -1.027, 0, 0, 0]
	parking_spots[15] = [-1.36, -0.6869999999999999, 0, 0, 0]
	parking_spots[16] = [-1.36, -0.347, 0, 0, 0]
	parking_spots[17] = [-1.4, 0.603, 0, 0, 0]
	parking_spots[18] = [-0.6399999999999999, 0.32999999999999985, 0, 0, 0]
	parking_spots[19] = [-0.6399999999999999, 0.6500000000000001, 0, 0, 0]
	parking_spots[20] = [-1.4, 0.9529999999999998, 0, 0, 0]
	parking_spots[21] = [-0.6399999999999999, 1.2899999999999998, 0, 0, 0]
	parking_spots[22] = [-1.4, 1.6529999999999998, 0, 0, 0]
	parking_spots[23] = [-0.6399999999999999, 1.93, 0, 0, 0]
	parking_spots[24] = [-0.4, -2.1, 1.5707963267948966, 0, 0]
	parking_spots[25] = [-0.32999999999999996, -1.27, 0, 0, 0]
	parking_spots[26] = [-1.36, -0.857, 0, 0, 0]
	parking_spots[27] = [0.17999999999999994, -2.1, 1.5707963267948966, 0, 0]
	parking_spots[28] = [-0.32999999999999996, -0.31000000000000005, 0, 0, 0]
	parking_spots[29] = [-0.32999999999999996, 0.010000000000000009, 0, 0, 0]
	parking_spots[30] = [-0.32999999999999996, 0.32999999999999985, 0, 0, 0]
	parking_spots[31] = [0.32499999999999996, -2.1, 1.5707963267948966, 0, 0]
	parking_spots[32] = [-0.32999999999999996, 0.97, 0, 0, 0]
	parking_spots[33] = [-0.32999999999999996, 1.2899999999999998, 0, 0, 0]
	parking_spots[34] = [-1.4, 1.8279999999999998, 0, 0, 0]
	parking_spots[35] = [-1.4, 2.003, 0, 0, 0]
	parking_spots[36] = [0.03499999999999992, -2.1, 1.5707963267948966, 0, 0]
	parking_spots[37] = [0.46999999999999986, -2.1, 1.5707963267948966, 0, 0]
	parking_spots[38] = [0.6149999999999999, -2.1, 1.5707963267948966, 0, 0]
	parking_spots[39] = [-0.020000000000000018, -0.6300000000000001, 0, 0, 0]
	parking_spots[40] = [-1.36, -0.17699999999999982, 0, 0, 0]
	parking_spots[41] = [-1.36, -0.006999999999999895, 0, 0, 0]
	parking_spots[42] = [-1.36, 0.16300000000000003, 0, 0, 0]
	parking_spots[43] = [-1.36, -1.367, 0, 0, 0]
	parking_spots[44] = [-0.020000000000000018, 0.97, 0, 0, 0]
	parking_spots[45] = [-1.4, 1.303, 0, 0, 0]
	parking_spots[46] = [-1.4, 1.4779999999999998, 0, 0, 0]
	parking_spots[47] = [-0.020000000000000018, 1.93, 0, 0, 0]
	parking_spots[48] = [0.29000000000000004, -1.59, 0, 0, 0]
	parking_spots[49] = [0.7599999999999999, -2.1, 1.5707963267948966, 0, 0]
	parking_spots[50] = [0.29000000000000004, -0.9500000000000001, 0, 0, 0]
	parking_spots[51] = [0.29000000000000004, -0.6300000000000001, 0, 0, 0]
	parking_spots[52] = [0.29000000000000004, -0.31000000000000005, 0, 0, 0]
	parking_spots[53] = [0.29000000000000004, 0.010000000000000009, 0, 0, 0]
	parking_spots[54] = [0.29000000000000004, 0.32999999999999985, 0, 0, 0]
	parking_spots[55] = [0.29000000000000004, 0.6500000000000001, 0, 0, 0]
	parking_spots[56] = [0.29000000000000004, 0.97, 0, 0, 0]
	parking_spots[57] = [0.29000000000000004, 1.2899999999999998, 0, 0, 0]
	parking_spots[58] = [0.29000000000000004, 1.61, 0, 0, 0]
	parking_spots[59] = [0.29000000000000004, 1.93, 0, 0, 0]
	return parking_spots

def temp_poses(r_id):
    init_pose = {}
    init_pose[0] = [-0.3,0,0]
    init_pose[1] = [0.3,0,1.57]
    # init_pose[0] = [0,0,0]
    # init_pose[1] = [-0.25,0.4,0]
    # init_pose[2] = [-0.5,0.8,0]
    # init_pose[3] = [-0.75,1.2,0]
    # init_pose[4] = [0.25,-0.4,0]
    # init_pose[5] = [0.5,-0.9,0]
    # init_pose[6] = [-0.75,0.9,0]
    # init_pose[7] = [-0.75,0.6,0]
    # init_pose[8] = [-0.75,0.3,0]
    # init_pose[9] = [-0.75,0.0,0]
    # init_pose[10] = [-0.75,-0.3,0]
    # init_pose[11] = [-0.75,-0.6,0]
    # init_pose[12] = [-0.75,-0.9,0]
    # init_pose[13] = [0.5,-0.6,3.14]
    # init_pose[14] = [0.5,-0.3,3.14]
    # init_pose[15] = [0.5,-0.0,3.14]
    # init_pose[16] = [0.5,0.3,3.14]
    # init_pose[17] = [0.5,0.6,3.14]
    # init_pose[18] = [0.5,0.9,3.14]
    # init_pose[19] = [0.5,1.2,3.14]
    return init_pose[r_id]